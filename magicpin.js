...
(async () => {
  try {
    ...
    const csvContent = reviews
      .map(
        (review) =>
          `${review.name},${review.followers},${review.date},${review.review}`
      )
      .join("\n");
    fs.writeFileSync("reviews_magicpin.csv", csvContent);

    console.log("Reviews extracted and saved to reviews_magicpin.csv");
